...
OPS = {
    'none' : lambda C, stride, affine=False: Zero(stride),
    ...
    # kernel, stride, padding=kernel // 2 * dilation, dilation
    'conv_1x1': lambda in_channel, out_channel: ConvLayer(in_channel, out_channel, 1, 1, 0, 1),
    'conv_3x3': lambda in_channel, out_channel: ConvLayer(in_channel, out_channel, 3, 1, 1, 1),
    'conv_3x3_dil2': lambda in_channel, out_channel: ConvLayer(in_channel, out_channel, 3, 1, 2, 2),
    'conv_3x3_dil3': lambda in_channel, out_channel: ConvLayer(in_channel, out_channel, 3, 1, 3, 3),
    'conv_5x5': lambda in_channel, out_channel: ConvLayer(in_channel, out_channel, 5, 1, 2, 1),
    'conv_5x5_dil2': lambda in_channel, out_channel: ConvLayer(in_channel, out_channel, 5, 1, 4, 2),
    'se_layer': lambda in_channel, out_channel: SELayer(in_channel, out_channel),
}
...
class ConvLayer(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, stride, padding, dilation):
        ...
        self.op = nn.Sequential(
                nn.PReLU(),
                nn.Conv3d(in_channel, out_channel, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, bias=False),
...
class ReduceLayer(nn.Module):
  def __init__(self, C_in, C_out):
    ...
    self.conv = nn.Conv3d(C_in, C_out, 1, stride=2, padding=0, bias=False)
...
class ASPP(nn.Module):
    def __init__(self, in_channels, paddings, dilations, num_classes):
        ...
        self.conv11 = nn.Sequential(nn.Conv3d(in_channels, in_channels, 1, bias=False),
                                    nn.BatchNorm3d(in_channels))
        self.conv33 = nn.Sequential(nn.Conv3d(in_channels, in_channels, 3,
                                    padding=paddings, dilation=dilations, bias=False),
                                      nn.BatchNorm3d(in_channels))
        self.conv_p = nn.Sequential(nn.Conv3d(in_channels, in_channels, 1, bias=False),
                                    nn.BatchNorm3d(in_channels))

        self.concate_conv = nn.Sequential(nn.Conv3d(in_channels * 3, self._num_classes, 1,  stride=1, padding=0))
    ...
    def forward(self, x):
        ...
        image_pool = nn.AvgPool3d(kernel_size=x.size()[2:])
        image_pool = image_pool(x)
        upsample = nn.Upsample(size=x.size()[2:], mode='trilinear', align_corners=True)
